...
기본적으로, OLS에서는 RSS를 최소로 하는 것을 목표로 한다.
...
RSS를 최소로 하도록 계수를 학습한다면, 우항의 $\lambda \displaystyle\sum^p_{j=1} \beta^2_j$(<mark>shrinkage penalty</mark>)는 각 계수가 전부 0에 가까울 수록 값이 작아진다.
...
이렇듯, 좌측의 그래프 처럼 cv 를 통해 $\lambda$를 예측하게 되면, signal과 noises를 찾을 수 있으며,
